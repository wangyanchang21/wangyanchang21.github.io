...
又没看懂, 不要紧, 我们来分析一下: 
XCode 中创建一个基于 Objective-C 的 `iOS App` 工程。`Supporting Files`目录下会自动创建一个`main.m`文件。里面的代码段如下：
...
其实上面这段代码在OC工程中所做的事情, 就是 Swift 的项目中引入`@NSApplicationMain` 和 `@UIApplicationMain`两个特性所做的事情。
苹果对 Swift 的项目进行了简化，使用`@NSApplicationMain` 放在 `OSX` 桌面应用工程代理类前面，使用`@UIApplicationMain`放在 `iOS app` 工程代理类前面。与 OC 工程中的`main.m`的功能一模一样。这里我们使用 `iOS app`作为例子，`@NSApplicationMain` 在 `OSX` 桌面应用工程里面的用法是一样的, 当你使用Xcode 创建一个 Swift iOS 工程时，自动生成的 `AppDelegate.swift` 里面，会看到以下代码：
