...
在类中提供一个全能初始化方法，其他初始化方法均应调用此方法。而且, 每个子类的全能初始化方法都应该调用其父类的对应方法，并逐层向上。但是, 如果要调用`[[SomeClass alloc] init]`的话, 就需要处理一下了。一般的, 两种方式, 通过全能初始化方法设置默认值或者抛异常。
